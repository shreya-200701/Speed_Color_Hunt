...
# --- IF RUNNING AS POPUP SUBPROCESS ---
...
# CHANGED: borderless windowed fullscreen instead of exclusive pygame.FULLSCREEN.
...
def play_fail_sound():
    play_sound(220, 110, duration=0.3)
...
def launch_popup(final_score, max_rounds):
    # CHANGED: no more toggling fullscreen off/on — screen is already a
    # normal borderless window, so nothing special needs to happen before
    # spawning the popup subprocess.
    cmd = [sys.executable, __file__, "--popup", str(final_score), str(max_rounds)]
    result = subprocess.run(cmd, capture_output=True, text=True)
    output = result.stdout.strip()

    # CHANGED: surfaces any crash in the popup process instead of failing silently.
